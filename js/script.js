...
jQuery(document).ready(function() {
	...
	if( jQuery(".dribbble-widget").length > 0){
		jQuery('.dribbble-widget').each(function(){
			...
			else { jQuery.jribbble.setToken("c817dcce8d85095e0ee458e2b44735eeb5672629648c471f55ca74019f8b1bde"); }
...
jQuery(document).ready(function() {
	...
	// add google fonts for variation
	jQuery("head").append('<link href="http://fonts.googleapis.com/css?family=Montserrat:400,700" rel="stylesheet" type="text/css">');
	jQuery("head").append('<link href="http://fonts.googleapis.com/css?family=Roboto+Slab:700,300,400" rel="stylesheet" type="text/css">');

	jQuery("head").append('<link href="http://fonts.googleapis.com/css?family=Raleway:200,200italic,400,400italic,600,600italic" rel="stylesheet" type="text/css">');
	jQuery("head").append('<link href="http://fonts.googleapis.com/css?family=Lato:300,400,700" rel="stylesheet" type="text/css">');

	/*jQuery("head").append("<link rel='stylesheet' href='files/css/custom-font1.css' type='text/css'>");
	jQuery("head").append("<link rel='stylesheet' href='files/css/custom-font2.css' type='text/css'>");*/

	var defaultMenuWidget = '<div class="widget widget_social"><ul class="socialmedia-widget"><li class="facebook"><a href="#"></a></li><li class="twitter"><a href="#"></a></li><li class="behance"><a href="#"></a></li><li class="pinterest"><a href="#"></a></li></ul></div>';

	var columnMenuWidget = '<div class="widget widget_social"><ul class="socialmedia-widget"><li class="facebook"><a href="#"></a></li><li class="twitter"><a href="#"></a></li><li class="behance"><a href="#"></a></li><li class="pinterest"><a href="#"></a></li></ul></div><div class="widget widget_text"><h5 class="widget-title title-alt"><strong>About us</strong></h5><p>We provide perfect and professional services.  Covering areas of the modern communication.  We belive in good products.</p><p>Give us a call at <strong>12 212-568-999</strong><br>or contact us on <a href="http://www.spab-rice.com"><strong>contact@dani-theme.com</strong></a></p></div>';
	...
	jQuery('#sr-demo-panel').on("click", ".demo-button", function() {
		...
		if (option === "headercolor") {
			...
			if (value === "transparent") { jQuery("#header").removeClass("small-header"); }
			else {
				jQuery("#header").removeClass("text-light").removeClass("text-dark");
				if (jQuery(window).scrollTop() > 50) { jQuery("#header").addClass("small-header"); }
			}
			jQuery("#header").addClass(value);
		} else if (option === "menustyle") {
			jQuery("#header").removeClass("menu-default").removeClass("menu-open").removeClass("menu-full-center").removeClass("menu-full-columns");
			jQuery("#header").addClass(value);
		} else if (option === "headerwrapper") {
			jQuery("#header").removeClass("wrapper").removeClass("no-wrapper");
			jQuery("#header").addClass(value);
		} else if (option === "logocenter") {
			jQuery("#logo").removeClass("logo-normal").removeClass("logo-centered");
			jQuery("#logo").addClass(value);
		} else if (option === "headersticky") {
			jQuery("#header").removeClass("sticky").removeClass("not-sticky");
			jQuery("#header").addClass(value);
		}

		// # Dependecies
		if (option === "headercolor" && value === "transparent") {
			if (jQuery("#hero .active-revslide").hasClass("text-light") || jQuery("#hero").hasClass("text-light")) {
				jQuery("#header").addClass("text-light");
			}
		}

		if (option === "menustyle" && value === "menu-full-columns") { jQuery( "header #menu-widget" ).html(columnMenuWidget); }
		else if (option === "menustyle" && value !== "menu-full-columns") { jQuery( "header #menu-widget" ).html(defaultMenuWidget); }

		if (option === "menustyle" && value === "menu-open") {
			...
			jQuery('ul.sub-menu').removeClass('add-gap');
		}

		if (option === "logocenter" && value === "logo-centered") { jQuery( "header .header-inner" ).append('<div id="header-widget">'+columnMenuWidget+'</div>'); }
		else if (option === "logocenter" && value !== "logo-centered") { jQuery( "header .header-inner #header-widget" ).remove(); }

		if (option === "coloroptions" && value === "colored") { jQuery('head').append('<link id="colorstyle" rel="stylesheet" href="files/css/color.css" type="text/css" />'); }
		else if (option === "coloroption" && value !== "colored") { jQuery("#colorstyle").remove(); }
	...
	// Check default state
	if (jQuery('#header').hasClass("text-light") && !jQuery('#header').hasClass("transparent")) {
	...
	if (jQuery('#header').hasClass("transparent")) {
		jQuery( "#sr-demo-panel .demo-button[data-option='headercolor'][href='transparent']").trigger( "click" );
	}
	if (jQuery('#header').hasClass("wrapper")) {
		jQuery( "#sr-demo-panel .demo-button[data-option='headerwrapper'][href='wrapper']").trigger( "click" );
	}
	if (jQuery('#header').hasClass("not-sticky")) {
		jQuery( "#sr-demo-panel .demo-button[data-option='headersticky'][href='not-sticky']").trigger( "click" );
	}
	if (jQuery('#logo').hasClass("logo-centered")) {
		jQuery( "#sr-demo-panel .demo-button[data-option='logocenter'][href='logo-centered']").trigger( "click" );
	}
	// Check default state
